<template>
  <view class="wrapper">
    ...
    <view class="shipment" v-if="address">
      ...
      <view class="dd">{{ allAdress }}</view>
    </view>
    <button @click="getAdress" class="addressBtn" type="primary" v-else>
      获取收货地址
    </button>
    <!-- 购物车 -->
    <view class="carts">
...
export default {
  data () {
    return {
      // 问题：放到这里获取，只会在页面第一次加载时，从本地获取一次=》以后购物车有更新，这里不会获取到更新
      carts: [],
      // 微信用户收货地址信息
      address: null
    }
  ...
  methods: {
    ...
    async createOrder () {
      ...
      if (!uni.getStorageSync('ugo-token-150')) {
      ...
      const { msg, data } = await this.request({
        ...
        method: 'post',
        data: {
          order_price: this.seledGoodsTotal, // 选中商品总价
          consignee_addr: this.allAdress, // 完整拼接收货地址
      ...
      if (msg.status === 200) {
        /**
         * 订单创建成功：
         * jd购物车选中商品下单=》成功后=》从购物车删除选中的商品
         * 1. 下单=》成功后=》从购物车删除选中的商品
         * 2. 跳转订单列表页
         */
        // 获取购车中未选中商品
    ...
    selAll () {
      /**
       * 判断全选框是否是选中状态？取反
       * 1. true=》全选框选中 =》购物车列表中的商品应该是全部未选中
       * 2. false=》全选框未选中 =》购物车列表中的商品应该是全部选中
       */
      if (this.isAll) {
